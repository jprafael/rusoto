...
MarketplaceCommerceAnalytics(String),/// An error occurred dispatching the HTTP request
...
/// A client for the AWS Marketplace Commerce Analytics API.
        pub struct MarketplaceCommerceAnalyticsClient<P, D> where P: ProvideAwsCredentials, D: DispatchSignedRequest {
            credentials_provider: P,
            region: region::Region,
            dispatcher: D,
        }

        impl<P, D> MarketplaceCommerceAnalyticsClient<P, D> where P: ProvideAwsCredentials, D: DispatchSignedRequest {
        ...
        impl<P, D> MarketplaceCommerceAnalytics for MarketplaceCommerceAnalyticsClient<P, D> where P: ProvideAwsCredentials, D: DispatchSignedRequest {
                ...
                fn generate_data_set(&self, input: &GenerateDataSetRequest)  -> Box<Future<Item = GenerateDataSetResult, Error = GenerateDataSetError>> {
                    ...
                    request.add_header("x-amz-target", "MarketplaceCommerceAnalytics20150701.GenerateDataSet");
         ...
         request.set_payload(Some(encoded.into_bytes()));
                ...
                fn start_support_data_export(&self, input: &StartSupportDataExportRequest)  -> Box<Future<Item = StartSupportDataExportResult, Error = StartSupportDataExportError>> {
                    ...
                    request.set_content_type("application/x-amz-json-1.1".to_owned());
                    request.add_header("x-amz-target", "MarketplaceCommerceAnalytics20150701.StartSupportDataExport");
                    let encoded = serde_json::to_string(input).unwrap();
         request.set_payload(Some(encoded.into_bytes()));
