...
/// Trait representing the capabilities of the Kinesis API. Kinesis clients implement this trait.
        pub trait Kinesis {
                ...
                fn add_tags_to_stream(&self, input: &AddTagsToStreamInput)  -> Box<Future<Item = (), Error = AddTagsToStreamError>>;
                ...
                fn create_stream(&self, input: &CreateStreamInput)  -> Box<Future<Item = (), Error = CreateStreamError>>;
                ...
                fn decrease_stream_retention_period(&self, input: &DecreaseStreamRetentionPeriodInput)  -> Box<Future<Item = (), Error = DecreaseStreamRetentionPeriodError>>;
                ...
                fn delete_stream(&self, input: &DeleteStreamInput)  -> Box<Future<Item = (), Error = DeleteStreamError>>;
                ...
                fn describe_limits(&self, input: &DescribeLimitsInput)  -> Box<Future<Item = DescribeLimitsOutput, Error = DescribeLimitsError>>;
                ...
                fn disable_enhanced_monitoring(&self, input: &DisableEnhancedMonitoringInput)  -> Box<Future<Item = EnhancedMonitoringOutput, Error = DisableEnhancedMonitoringError>>;
                

                #[doc="<p>Enables enhanced Amazon Kinesis stream monitoring for shard-level metrics.</p>"]
                fn enable_enhanced_monitoring(&self, input: &EnableEnhancedMonitoringInput)  -> Box<Future<Item = EnhancedMonitoringOutput, Error = EnableEnhancedMonitoringError>>;
                ...
                fn increase_stream_retention_period(&self, input: &IncreaseStreamRetentionPeriodInput)  -> Box<Future<Item = (), Error = IncreaseStreamRetentionPeriodError>>;
                ...
                fn list_tags_for_stream(&self, input: &ListTagsForStreamInput)  -> Box<Future<Item = ListTagsForStreamOutput, Error = ListTagsForStreamError>>;
                ...
                fn merge_shards(&self, input: &MergeShardsInput)  -> Box<Future<Item = (), Error = MergeShardsError>>;
                ...
                fn put_record(&self, input: &PutRecordInput)  -> Box<Future<Item = PutRecordOutput, Error = PutRecordError>>;
                ...
                fn put_records(&self, input: &PutRecordsInput)  -> Box<Future<Item = PutRecordsOutput, Error = PutRecordsError>>;
                ...
                fn remove_tags_from_stream(&self, input: &RemoveTagsFromStreamInput)  -> Box<Future<Item = (), Error = RemoveTagsFromStreamError>>;
                ...
                fn split_shard(&self, input: &SplitShardInput)  -> Box<Future<Item = (), Error = SplitShardError>>;
                ...
                fn update_shard_count(&self, input: &UpdateShardCountInput)  -> Box<Future<Item = UpdateShardCountOutput, Error = UpdateShardCountError>>;
                
}
/// A client for the Kinesis API.
        pub struct KinesisClient<P, D> where P: ProvideAwsCredentials, D: DispatchSignedRequest {
            credentials_provider: P,
            region: region::Region,
            dispatcher: D,
        }

        impl<P, D> KinesisClient<P, D> where P: ProvideAwsCredentials, D: DispatchSignedRequest {
        ...
        impl<P, D> Kinesis for KinesisClient<P, D> where P: ProvideAwsCredentials, D: DispatchSignedRequest {
         ...
         request.set_payload(Some(encoded.into_bytes()));
                    ...
                    let credentials = match self.credentials_provider.credentials() {
                        Ok(c) => c,
                        Err(err) => return Box::new(future::err(AddTagsToStreamError::from(err)))
                    };

                    request.sign(&credentials);

                    let res = self.dispatcher.dispatch(&request)
                        .map_err(|dispatch_err| AddTagsToStreamError::from(dispatch_err))
                        .and_then(
                            |response| match response.status {
                                StatusCode::Ok => {
                                    future::ok(())
                                }
                                _ => future::err(AddTagsToStreamError::from_body(String::from_utf8_lossy(&response.body).as_ref())),
                            }
                        );
         ...
         request.set_payload(Some(encoded.into_bytes()));
                    ...
                    let credentials = match self.credentials_provider.credentials() {
                        Ok(c) => c,
                        Err(err) => return Box::new(future::err(CreateStreamError::from(err)))
                    };

                    request.sign(&credentials);

                    let res = self.dispatcher.dispatch(&request)
                        .map_err(|dispatch_err| CreateStreamError::from(dispatch_err))
                        .and_then(
                            |response| match response.status {
                                StatusCode::Ok => {
                                    future::ok(())
                                }
                                _ => future::err(CreateStreamError::from_body(String::from_utf8_lossy(&response.body).as_ref())),
                            }
                        );
                ...
                fn decrease_stream_retention_period(&self, input: &DecreaseStreamRetentionPeriodInput)  -> Box<Future<Item = (), Error = DecreaseStreamRetentionPeriodError>> {
                    ...
                    request.set_content_type("application/x-amz-json-1.1".to_owned());
                    request.add_header("x-amz-target", "Kinesis_20131202.DecreaseStreamRetentionPeriod");
                    let encoded = serde_json::to_string(input).unwrap();
         request.set_payload(Some(encoded.into_bytes()));
                    ...
                    let credentials = match self.credentials_provider.credentials() {
                        Ok(c) => c,
                        Err(err) => return Box::new(future::err(DecreaseStreamRetentionPeriodError::from(err)))
                    };

                    request.sign(&credentials);

                    let res = self.dispatcher.dispatch(&request)
                        .map_err(|dispatch_err| DecreaseStreamRetentionPeriodError::from(dispatch_err))
                        .and_then(
                            |response| match response.status {
                                StatusCode::Ok => {
                                    future::ok(())
                                }
                                _ => future::err(DecreaseStreamRetentionPeriodError::from_body(String::from_utf8_lossy(&response.body).as_ref())),
                            }
                        );
         ...
         request.set_payload(Some(encoded.into_bytes()));
                    ...
                    let credentials = match self.credentials_provider.credentials() {
                        Ok(c) => c,
                        Err(err) => return Box::new(future::err(DeleteStreamError::from(err)))
                    };

                    request.sign(&credentials);

                    let res = self.dispatcher.dispatch(&request)
                        .map_err(|dispatch_err| DeleteStreamError::from(dispatch_err))
                        .and_then(
                            |response| match response.status {
                                StatusCode::Ok => {
                                    future::ok(())
                                }
                                _ => future::err(DeleteStreamError::from_body(String::from_utf8_lossy(&response.body).as_ref())),
                            }
                        );
         ...
         request.set_payload(Some(encoded.into_bytes()));
                    ...
                    let credentials = match self.credentials_provider.credentials() {
                        Ok(c) => c,
                        Err(err) => return Box::new(future::err(DescribeLimitsError::from(err)))
                    };

                    request.sign(&credentials);

                    let res = self.dispatcher.dispatch(&request)
                        .map_err(|dispatch_err| DescribeLimitsError::from(dispatch_err))
                        .and_then(
                            |response| match response.status {
                                StatusCode::Ok => {
                                    future::ok(serde_json::from_str::<DescribeLimitsOutput>(String::from_utf8_lossy(&response.body).as_ref()).unwrap())
                                }
                                _ => future::err(DescribeLimitsError::from_body(String::from_utf8_lossy(&response.body).as_ref())),
                            }
                        );
         ...
         request.set_payload(Some(encoded.into_bytes()));
                    ...
                    let credentials = match self.credentials_provider.credentials() {
                        Ok(c) => c,
                        Err(err) => return Box::new(future::err(DisableEnhancedMonitoringError::from(err)))
                    };

                    request.sign(&credentials);

                    let res = self.dispatcher.dispatch(&request)
                        .map_err(|dispatch_err| DisableEnhancedMonitoringError::from(dispatch_err))
                        .and_then(
                            |response| match response.status {
                                StatusCode::Ok => {
                                    future::ok(serde_json::from_str::<EnhancedMonitoringOutput>(String::from_utf8_lossy(&response.body).as_ref()).unwrap())
                                }
                                _ => future::err(DisableEnhancedMonitoringError::from_body(String::from_utf8_lossy(&response.body).as_ref())),
                            }
                        );
         ...
         request.set_payload(Some(encoded.into_bytes()));
                    ...
                    let credentials = match self.credentials_provider.credentials() {
                        Ok(c) => c,
                        Err(err) => return Box::new(future::err(EnableEnhancedMonitoringError::from(err)))
                    };

                    request.sign(&credentials);

                    let res = self.dispatcher.dispatch(&request)
                        .map_err(|dispatch_err| EnableEnhancedMonitoringError::from(dispatch_err))
                        .and_then(
                            |response| match response.status {
                                StatusCode::Ok => {
                                    future::ok(serde_json::from_str::<EnhancedMonitoringOutput>(String::from_utf8_lossy(&response.body).as_ref()).unwrap())
                                }
                                _ => future::err(EnableEnhancedMonitoringError::from_body(String::from_utf8_lossy(&response.body).as_ref())),
                            }
                        );
         ...
         request.set_payload(Some(encoded.into_bytes()));
                ...
                fn increase_stream_retention_period(&self, input: &IncreaseStreamRetentionPeriodInput)  -> Box<Future<Item = (), Error = IncreaseStreamRetentionPeriodError>> {
                    ...
                    request.set_content_type("application/x-amz-json-1.1".to_owned());
                    request.add_header("x-amz-target", "Kinesis_20131202.IncreaseStreamRetentionPeriod");
                    let encoded = serde_json::to_string(input).unwrap();
         request.set_payload(Some(encoded.into_bytes()));
                    ...
                    let credentials = match self.credentials_provider.credentials() {
                        Ok(c) => c,
                        Err(err) => return Box::new(future::err(IncreaseStreamRetentionPeriodError::from(err)))
                    };

                    request.sign(&credentials);

                    let res = self.dispatcher.dispatch(&request)
                        .map_err(|dispatch_err| IncreaseStreamRetentionPeriodError::from(dispatch_err))
                        .and_then(
                            |response| match response.status {
                                StatusCode::Ok => {
                                    future::ok(())
                                }
                                _ => future::err(IncreaseStreamRetentionPeriodError::from_body(String::from_utf8_lossy(&response.body).as_ref())),
                            }
                        );
         ...
         request.set_payload(Some(encoded.into_bytes()));
                    ...
                    let credentials = match self.credentials_provider.credentials() {
                        Ok(c) => c,
                        Err(err) => return Box::new(future::err(ListTagsForStreamError::from(err)))
                    };

                    request.sign(&credentials);

                    let res = self.dispatcher.dispatch(&request)
                        .map_err(|dispatch_err| ListTagsForStreamError::from(dispatch_err))
                        .and_then(
                            |response| match response.status {
                                StatusCode::Ok => {
                                    future::ok(serde_json::from_str::<ListTagsForStreamOutput>(String::from_utf8_lossy(&response.body).as_ref()).unwrap())
                                }
                                _ => future::err(ListTagsForStreamError::from_body(String::from_utf8_lossy(&response.body).as_ref())),
                            }
                        );
         ...
         request.set_payload(Some(encoded.into_bytes()));
                    ...
                    let credentials = match self.credentials_provider.credentials() {
                        Ok(c) => c,
                        Err(err) => return Box::new(future::err(MergeShardsError::from(err)))
                    };

                    request.sign(&credentials);

                    let res = self.dispatcher.dispatch(&request)
                        .map_err(|dispatch_err| MergeShardsError::from(dispatch_err))
                        .and_then(
                            |response| match response.status {
                                StatusCode::Ok => {
                                    future::ok(())
                                }
                                _ => future::err(MergeShardsError::from_body(String::from_utf8_lossy(&response.body).as_ref())),
                            }
                        );
         ...
         request.set_payload(Some(encoded.into_bytes()));
                    ...
                    let credentials = match self.credentials_provider.credentials() {
                        Ok(c) => c,
                        Err(err) => return Box::new(future::err(PutRecordError::from(err)))
                    };

                    request.sign(&credentials);

                    let res = self.dispatcher.dispatch(&request)
                        .map_err(|dispatch_err| PutRecordError::from(dispatch_err))
                        .and_then(
                            |response| match response.status {
                                StatusCode::Ok => {
                                    future::ok(serde_json::from_str::<PutRecordOutput>(String::from_utf8_lossy(&response.body).as_ref()).unwrap())
                                }
                                _ => future::err(PutRecordError::from_body(String::from_utf8_lossy(&response.body).as_ref())),
                            }
                        );
         ...
         request.set_payload(Some(encoded.into_bytes()));
                    ...
                    let credentials = match self.credentials_provider.credentials() {
                        Ok(c) => c,
                        Err(err) => return Box::new(future::err(PutRecordsError::from(err)))
                    };

                    request.sign(&credentials);

                    let res = self.dispatcher.dispatch(&request)
                        .map_err(|dispatch_err| PutRecordsError::from(dispatch_err))
                        .and_then(
                            |response| match response.status {
                                StatusCode::Ok => {
                                    future::ok(serde_json::from_str::<PutRecordsOutput>(String::from_utf8_lossy(&response.body).as_ref()).unwrap())
                                }
                                _ => future::err(PutRecordsError::from_body(String::from_utf8_lossy(&response.body).as_ref())),
                            }
                        );
         ...
         request.set_payload(Some(encoded.into_bytes()));
                    ...
                    let credentials = match self.credentials_provider.credentials() {
                        Ok(c) => c,
                        Err(err) => return Box::new(future::err(RemoveTagsFromStreamError::from(err)))
                    };

                    request.sign(&credentials);

                    let res = self.dispatcher.dispatch(&request)
                        .map_err(|dispatch_err| RemoveTagsFromStreamError::from(dispatch_err))
                        .and_then(
                            |response| match response.status {
                                StatusCode::Ok => {
                                    future::ok(())
                                }
                                _ => future::err(RemoveTagsFromStreamError::from_body(String::from_utf8_lossy(&response.body).as_ref())),
                            }
                        );
         ...
         request.set_payload(Some(encoded.into_bytes()));
                    ...
                    let credentials = match self.credentials_provider.credentials() {
                        Ok(c) => c,
                        Err(err) => return Box::new(future::err(SplitShardError::from(err)))
                    };

                    request.sign(&credentials);

                    let res = self.dispatcher.dispatch(&request)
                        .map_err(|dispatch_err| SplitShardError::from(dispatch_err))
                        .and_then(
                            |response| match response.status {
                                StatusCode::Ok => {
                                    future::ok(())
                                }
                                _ => future::err(SplitShardError::from_body(String::from_utf8_lossy(&response.body).as_ref())),
                            }
                        );
         ...
         request.set_payload(Some(encoded.into_bytes()));
                    ...
                    let credentials = match self.credentials_provider.credentials() {
                        Ok(c) => c,
                        Err(err) => return Box::new(future::err(UpdateShardCountError::from(err)))
                    };

                    request.sign(&credentials);

                    let res = self.dispatcher.dispatch(&request)
                        .map_err(|dispatch_err| UpdateShardCountError::from(dispatch_err))
                        .and_then(
                            |response| match response.status {
                                StatusCode::Ok => {
                                    future::ok(serde_json::from_str::<UpdateShardCountOutput>(String::from_utf8_lossy(&response.body).as_ref()).unwrap())
                                }
                                _ => future::err(UpdateShardCountError::from_body(String::from_utf8_lossy(&response.body).as_ref())),
                            }
                        );
